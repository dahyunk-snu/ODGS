	...
	float* __restrict__ out_acc)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float2 dscale;
			float2 d_corr = correctedOmniDelta(d, xy, pixf, H, dscale);
			// Radius/tile culling still uses the center covariance, so
			// strongly corrected footprints may need a larger future bound.
			float power = -0.5f * (con_o.x * d_corr.x * d_corr.x + con_o.z * d_corr.y * d_corr.y) - con_o.y * d_corr.x * d_corr.y;
			if (power > 0.0f)
				continue;
